      ...
      <div className="flex items-center gap-1 border-b border-border">
        {(["all", "TODO", "IN_PROGRESS", "IN_REVIEW", "DONE"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            {f === "all" ? "All Tasks" : f.replace("_", " ").toLowerCase()}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-1">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground capitalize">{task.type.toLowerCase()}</span>
                    </div>
                  </div>
